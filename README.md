...
學生在登入頁輸入隊伍名稱與密碼即可進入。如果隊伍名稱尚不存在，系統會自動建立該隊伍，並把本次輸入的密碼設定為隊伍密碼。之後同一隊伍登入需要輸入相同密碼。教師可在後台修改隊伍名稱、密碼、啟用狀態或刪除隊伍。

## 權限規則

- 未登入只能看到登入頁。
- 教師登入後可進入教師後台與大螢幕。
- 隊伍第一次登入可自建隊伍並設定密碼；之後登入需要輸入同一組密碼。
- 隊伍登入後只能進入競賽頁與大螢幕。
